#!/usr/bin/env python
"""
CoinMarketCap Pro v3 MCP Server Health Check Script

This script properly connects to the CoinMarketCap Pro v3 MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if CoinMarketCap Pro v3 MCP server is healthy"""
    print(f"🔍 Checking CoinMarketCap Pro v3 MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected CoinMarketCap Pro v3 tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check CoinMarketCap Pro v3 MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="CoinMarketCap Pro v3 MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 CoinMarketCap Pro v3 MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ CoinMarketCap Pro v3 MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ CoinMarketCap Pro v3 MCP Server health check failed!")
        return 1
