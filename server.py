#!/usr/bin/env python3
"""
CoinMarketCap Pro v3 MCP Server - FastMCP with D402 Transport Wrapper
...
- COINMARKETCAP_PRO_V3_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('coinmarketcap-pro-v3_mcp')
...
API_KEY = os.getenv("COINMARKETCAP_PRO_V3_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINMARKETCAP_PRO_V3_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"CoinMarketCap Pro v3 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("CoinMarketCap Pro v3 MCP Server", host="0.0.0.0")
...
# - COINMARKETCAP_PRO_V3_API_KEY: Server's internal API key for payment mode
#
...
async def content_latest(
    ...
    Args:
        ...
        id: Optionally pass a comma-separated list of CoinMarketCap cryptocurrency IDs. Example: "1,1027" (optional) Examples: "1", "1027"
        slug: Optionally pass a comma-separated list of cryptocurrency slugs. Example: "bitcoin,ethereum" (optional) Examples: "bitcoin", "ethereum"
        symbol: Optionally pass a comma-separated list of cryptocurrency symbols. Example: "BTC,ETH". Optionally pass "id" *or* "slug" *or* "symbol" is required for this request. (optional) Examples: "BTC", "ETH"
        ...
        category: Optionally pass a comma-separated list of categories. Example: "GameFi,NFT". (optional) Examples: "GameFi", "NFT"
...
async def coinmarketcap_id_map_kdbh(
    context: Context,
    listing_status: str = "active",
    slug: Optional[str] = None,
    start: int = 1,
    ...
    Example Usage:
        await coinmarketcap_id_map_kdbh()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_kdbh: {e}")
...
async def coinmarketcap_id_map_xbta(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_xbta()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_xbta: {e}")
...
async def price_conversion_v1_deprecated(
    ...
    Example Usage:
        await price_conversion_v1_deprecated(amount="10")
...
async def price_conversion_v2(
    ...
    Example Usage:
        await price_conversion_v2(amount="10")
...
async def content_post_comments(
    ...
    Args:
        context: MCP context (auto-injected by framework, not user-provided)
        post_id: Required post ID. Example: 325670123 (optional) Examples: "325670123"

    Returns:
        API response (dict, list, or other JSON type)

    Example Usage:
        await content_post_comments(post_id="325670123")
...
async def content_latest_posts(
    ...
    Args:
        context: MCP context (auto-injected by framework, not user-provided)
        id: Optional one cryptocurrency CoinMarketCap ID. Example: 1027 (optional) Examples: "1027"
        slug: Alternatively pass one cryptocurrency slug. Example: "ethereum" (optional) Examples: "ethereum"
        symbol: Alternatively pass one cryptocurrency symbols. Example: "ETH" (optional) Examples: "ETH"
        last_score: Optional. The score is given in the response for finding next batch posts. Example: 1662903634322 (optional) Examples: "1662903634322"
...
async def content_top_posts(
    ...
    Args:
        ...
        slug: Alternatively pass one cryptocurrency slug. Example: "ethereum" (optional) Examples: "ethereum"
        symbol: Alternatively pass one cryptocurrency symbols. Example: "ETH" (optional) Examples: "ETH"
        last_score: Optional. The score is given in the response for finding next batch of related posts. Example: 38507.8865 (optional) Examples: "38507"
...
async def listings_historical(
    ...
    Example Usage:
        await listings_historical(date="test")
...
async def listings_latest_nt2q(
    context: Context,
    start: int = 1,
    limit: int = 100,
    sort: str = "volume_24h",
    ...
    try:
        ...
        logger.error(f"Error in listings_latest_nt2q: {e}")
...
async def quotes_historical_7arl(
    context: Context,
    time_start: Optional[str] = None,
    time_end: Optional[str] = None,
    count: float = 10,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_7arl: {e}")
...
async def quotes_latest_7m6l(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_7m6l: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="coinmarketcap-pro-v3-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "coinmarketcap-pro-v3-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting CoinMarketCap Pro v3 MCP Server")
    logger.info("="*80)
