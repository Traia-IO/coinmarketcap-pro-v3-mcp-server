# CoinMarketCap Pro v3 MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the CoinMarketCap Pro v3 API. It enables AI agents and LLMs to interact with CoinMarketCap Pro v3 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with CoinMarketCap Pro v3 endpoints
...
- **CoinMarketCap Pro v3 Website**: [https://pro-api.coinmarketcap.com](https://pro-api.coinmarketcap.com)
...
*Note: Replace `example_tool` with actual CoinMarketCap Pro v3 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/coinmarketcap-pro-v3-mcp-server.git
   cd coinmarketcap-pro-v3-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINMARKETCAP_PRO_V3_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   COINMARKETCAP_PRO_V3_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
COINMARKETCAP_PRO_V3_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own CoinMarketCap Pro v3 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINMARKETCAP_PRO_V3_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their CoinMarketCap Pro v3 API key
2. Server uses client's API key to call CoinMarketCap Pro v3 API
3. No payment required
4. Client pays CoinMarketCap Pro v3 directly
...
6. Server uses its INTERNAL CoinMarketCap Pro v3 API key to call the API
...
```bash
# Required
COINMARKETCAP_PRO_V3_API_KEY=your_internal_coinmarketcap-pro-v3_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has CoinMarketCap Pro v3 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for CoinMarketCap Pro v3 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/coinmarketcap-pro-v3-mcp-server",
  "mcp_server": {
    "name": "coinmarketcap-pro-v3-mcp",
    "description": "Cryptocurrency market data, rankings, and metrics from coinmarketcap pro api",
...
- `COINMARKETCAP_PRO_V3_API_KEY`: Your CoinMarketCap Pro v3 API key (required)
## Troubleshooting
