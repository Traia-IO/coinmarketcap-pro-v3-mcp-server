# CoinMarketCap Pro v3 MCP Server Implementation Guide

You are working on implementing the CoinMarketCap Pro v3 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: CoinMarketCap Pro v3
- **Documentation**: [None](None)
- **Website**: [https://pro-api.coinmarketcap.com](https://pro-api.coinmarketcap.com)
- **Authentication**: Required - API key via `COINMARKETCAP_PRO_V3_API_KEY` environment variable
...
When a client provides their own CoinMarketCap Pro v3 API key:
...
Flow:
  1. Client connects with their CoinMarketCap Pro v3 API key
  2. MCP server uses client's API key to call CoinMarketCap Pro v3 API
  3. No payment required
  4. Client pays CoinMarketCap Pro v3 directly (not the MCP server)
```

**Use Case**: Clients who already have a CoinMarketCap Pro v3 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL CoinMarketCap Pro v3 API key to call the API
  5. Client pays the MCP server (not CoinMarketCap Pro v3)
```

**Use Case**: Pay-per-use clients without their own CoinMarketCap Pro v3 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINMARKETCAP_PRO_V3_API_KEY")
...
**Required**:
- `COINMARKETCAP_PRO_V3_API_KEY`: Server's internal CoinMarketCap Pro v3 API key (used when clients pay via 402)
...
COINMARKETCAP_PRO_V3_API_KEY=your_coinmarketcap-pro-v3_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_coinmarketcap_pro_v3",
      "get_coinmarketcap_pro_v3_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["coinmarketcap pro v3", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the CoinMarketCap Pro v3 API with retry support. Example pattern:
...
def call_coinmarketcap_pro_v3_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call CoinMarketCap Pro v3 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"CoinMarketCap Pro v3 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_coinmarketcap_pro_v3(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search CoinMarketCap Pro v3 for [specific data type].
    ...
    try:
        # The call_coinmarketcap_pro_v3_api function already has retry logic
        results = call_coinmarketcap_pro_v3_api(
...
```python
@mcp.tool()
async def get_coinmarketcap_pro_v3_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_coinmarketcap_pro_v3_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in CoinMarketCap Pro v3.
...
- **Log retry attempts** for debugging:
  ...
  def call_coinmarketcap_pro_v3_api(...):
      # Implementation
  ```
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coinmarketcap_pro_v3(query="example", limit=5)
   ```
...
### 9. Common CoinMarketCap Pro v3 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on CoinMarketCap Pro v3 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the CoinMarketCap Pro v3 documentation that you should implement:
...
- Check the CoinMarketCap Pro v3 API documentation: None
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make CoinMarketCap Pro v3's capabilities easily accessible to AI agents through standardized MCP tools. 