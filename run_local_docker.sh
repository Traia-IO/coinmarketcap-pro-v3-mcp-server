...
# Script to build and run the CoinMarketCap Pro v3 MCP Server locally in Docker
...
IMAGE_NAME="coinmarketcap-pro-v3-mcp-server"
CONTAINER_NAME="coinmarketcap-pro-v3-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running CoinMarketCap Pro v3 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# CoinMarketCap Pro v3 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
COINMARKETCAP_PRO_V3_API_KEY=
...
if ! grep -q "^COINMARKETCAP_PRO_V3_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your CoinMarketCap Pro v3 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "COINMARKETCAP_PRO_V3_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^COINMARKETCAP_PRO_V3_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^COINMARKETCAP_PRO_V3_API_KEY=.*|COINMARKETCAP_PRO_V3_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^COINMARKETCAP_PRO_V3_API_KEY=.*|COINMARKETCAP_PRO_V3_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "COINMARKETCAP_PRO_V3_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set COINMARKETCAP_PRO_V3_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ COINMARKETCAP_PRO_V3_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ CoinMarketCap Pro v3 MCP Server is running!${NC}"
echo
